...
const events = [
  ...
  },
  {
    ...
    eventDescription:
      "even even cooler and way wayyyyy more descriptive description",
    eventIsDone: false,
...
export default function Events() {
  ...
  const filteredPastEvents =
    activeTag == "All"
      ? pastEvents
      : pastEvents.filter((event) => event.eventTag == activeTag);

  return (
    <>
      <div className="flex flex-col items-center justify-center">
        <div className="flex flex-col w-4/5" id="upcoming-events-section">
          <h1 className="text-3xl font-bold my-6">Upcoming Events</h1>
        </div>
        <div className="flex flex-wrap-reverse justify-end w-4/5 mb-3 gap-4">
          <button
            onClick={() => setActiveTag("All")} //onClick calls setActiveTag function, useState updates - react rerenders - activeTag returns new value
            className={`w-fit text-sm text-gray-400 hover:text-white cursor-pointer 
                        ${activeTag == "All" ? "text-white font-bold" : ""}`}
          >
            all
          </button>
          <button
            onClick={() => setActiveTag("Social")}
            className={`w-fit text-sm text-gray-400 hover:text-white cursor-pointer 
          ...
          <button
            onClick={() => setActiveTag("Competition")}
            className={`w-fit text-sm text-gray-400 hover:text-white cursor-pointer 
        ...
        <div className="flex flex-col w-4/5" id="past-events-section">
          <h1 className="text-3xl font-bold my-6">Past Events</h1>
        </div>
